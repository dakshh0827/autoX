...
async def run_agent(request: AuthRunRequest) -> AgentResponse:
    logger.info(f"POST /run — topic='{request.topic}' user='{request.username}'")
    try:
        ...
        if not auth_result.success or not auth_result.storage_state_b64:
            raise HTTPException(
                status_code=401,
                detail=auth_result.message or "Authentication failed.",
            )
        ...
        result = await agent.run(
            AgentRequest(
                topic=request.topic,
                auth_storage_state_b64=auth_result.storage_state_b64,
            )
        )
